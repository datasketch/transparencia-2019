shinyServer(function(input, output, session) {
  ...
  output$base_opts <- renderUI({
    data_dis <- setNames(c("Territorios de concentración/consolidación", "informe II 2016-2018"), c("Territorios de paz", "Informe 2016-2018"))
    checkboxGroupInput("base_cliente", HTML("<div style='font-weight: 700;'>Selecciona información de interés</div>"), choices = data_dis, inline = T, selected = "informe II 2016-2018")
  })
  ...
  data_red <- reactive({
    ...
    if (var_cat_sel == "" | var_red_sel == "" | is.null(var_cat_sel) | is.null(var_red_sel)) {
      dt <- actores_data()
    } else {
      dt <-  actores[actores[, var_red_sel] == c(var_cat_sel), ]
    }
  ...
  nodes <- reactive({
    ...
    nodes$color <- "#C24224"
    nodes$color[nodes$type == "Caso"] <- "#43AFD0"
  ...
  output$netViz <- renderD3plus({
    viz <- d3plus(edges(), "network",nodes = nodes(), showTooltip = FALSE)
    viz
  })
  ...
  output$viz_res <- renderUI({
    ...
    if (l_o != 'Red') {
     ...
     v <- d3plusOutput("netViz", height = 550)
    }
    v
  })
